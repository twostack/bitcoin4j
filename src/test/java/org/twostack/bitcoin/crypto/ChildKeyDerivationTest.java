...
package org.twostack.bitcoin.crypto;
...
public class ChildKeyDerivationTest {
    private static final NetworkType MAINNET = NetworkType.MAIN;
    private static final NetworkType TESTNET =  NetworkType.TEST;
    private static final NetworkType UNITTEST = NetworkType.TEST;
    ...
    public void serializeToTextAndBytes() {
        ...
        {
            final String pub58 = key1.serializePubB58(networkType);
            final String priv58 = key1.serializePrivB58(networkType);
            final byte[] pub = key1.serializePublic(networkType);
            final byte[] priv = key1.serializePrivate(networkType);
            assertEquals("xpub661MyMwAqRbcF7mq7Aejj5xZNzFfgi3ABamE9FedDHVmViSzSxYTgAQGcATDo2J821q7Y9EAagjg5EP3L7uBZk11PxZU3hikL59dexfLkz3", pub58);
            assertEquals("xprv9s21ZrQH143K2dhN197jMx1ppxRBHFKJpMqdLsF1ewxncv7quRED8N5nksxphju3W7naj1arF56L5PUEWfuSk8h73Sb2uh7bSwyXNrjzhAZ", priv58);
            ...
            assertEquals(DeterministicKey.deserializeB58(null, priv58, networkType), key1);
            assertEquals(DeterministicKey.deserializeB58(priv58, networkType), key1);
            assertEquals(DeterministicKey.deserializeB58(null, pub58, networkType).getPubKeyPoint(), key1.getPubKeyPoint());
            assertEquals(DeterministicKey.deserializeB58(pub58, networkType).getPubKeyPoint(), key1.getPubKeyPoint());
            assertEquals(DeterministicKey.deserialize(networkType, priv, null), key1);
            assertEquals(DeterministicKey.deserialize(networkType, priv), key1);
            assertEquals(DeterministicKey.deserialize(networkType, pub, null).getPubKeyPoint(), key1.getPubKeyPoint());
            assertEquals(DeterministicKey.deserialize(networkType, pub).getPubKeyPoint(), key1.getPubKeyPoint());
        }
        {
            final String pub58 = key2.serializePubB58(networkType);
            final String priv58 = key2.serializePrivB58(networkType);
            final byte[] pub = key2.serializePublic(networkType);
            final byte[] priv = key2.serializePrivate(networkType);
            assertEquals(DeterministicKey.deserializeB58(key1, priv58, networkType), key2);
            assertEquals(DeterministicKey.deserializeB58(key1, pub58, networkType).getPubKeyPoint(), key2.getPubKeyPoint());
            assertEquals(DeterministicKey.deserialize(networkType, priv, key1), key2);
            assertEquals(DeterministicKey.deserialize(networkType, pub, key1).getPubKeyPoint(), key2.getPubKeyPoint());
        }
    }
