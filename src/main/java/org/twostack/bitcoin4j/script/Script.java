    ...
    public static Script fromBitcoindAsmString(String string) throws IOException {
        ...
        for(String w : words) {
            ...
            if (w.matches("^-?[0-9]*$")) {
                ...
                if (val >= -1 && val <= 16) {
                    builder.op(Script.encodeToOpN((int) val));
                } else{
                    //FIXME: Confirm that we can properly read pushed bigints this way
                    builder.bigNum(val);
                }
            } else if (w.matches("^0x[0-9a-fA-F]*$")) {
                // Raw hex data
                builder.data(HEX.decode(w.substring(2).toLowerCase()));
            } else if (w.length() >= 2 && w.startsWith("'") && w.endsWith("'")) {
                // Single-quoted string, pushed as data. NOTE: this is poor-man's
                // parsing, spaces/tabs/newlines in single-quoted strings won't work.
                builder.data(w.substring(1, w.length() - 1).getBytes(StandardCharsets.UTF_8));
            } else if (ScriptOpCodes.getOpCode(w) != OP_INVALIDOPCODE) {
                // opcode, e.g. OP_ADD or OP_1:
                builder.op(ScriptOpCodes.getOpCode(w));
            } else if (w.startsWith("OP_") && ScriptOpCodes.getOpCode(w.substring(3)) != OP_INVALIDOPCODE) {
                // opcode, e.g. OP_ADD or OP_1:
                builder.op(ScriptOpCodes.getOpCode(w.substring(3)));
            } else {
